...
export function ThemeProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    // Load saved theme color and mode after initial mount
    // Using setTimeout to move state updates to the next tick
    // which prevents the "cascading renders" error in React.
    const timeoutId = setTimeout(() => {
      ...
      if (savedMode) {
        ...
        // Default to dark mode if no saved mode
        applyThemeMode("dark");
      }
    }, 0);
