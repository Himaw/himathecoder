...
const projects = [
  {
    ...
    description:
      "that can be used for generating high quality YouTube Thumbnails, boosting video impressions and engagement.",
  ...
  },
  {
    ...
    description:
      "for secure, accessible education representing a cutting-edge solution that leverages principles of decentralization.",
  ...
  },
  {
    ...
    description:
      "for a real time Indoor positioning system, which visualizes the realtime location data.",
    image: "/img/conn.png",
  ...
  },
  {
    ...
    description:
      "for a printing press, with a modern design that provides a fully responsive and user-friendly experience.",
...
function ProjectCard({
  ...
  return (
    <motion.div
      ...
      className="group relative h-full overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--card)] backdrop-blur-sm transition-all duration-500 hover:border-primary/30"
    >
      <div className="flex flex-col sm:flex-row h-full min-h-[128px] md:min-h-[160px]">
        ...
        <div className="flex flex-1 flex-col p-6">
          <div className="flex flex-wrap gap-2 mb-3">
            {project.stack.map((tech) => (
              <span
                key={tech}
                className="rounded-full border border-[var(--border)] bg-[var(--card)] px-3 py-1 text-[10px] md:text-[10px] font-bold uppercase tracking-widest text-primary/80"
              >
                {tech}
              </span>
            ))}
          ...
          <div className="mb-2">
            <h3 className="font-display text-xl md:text-xl font-black uppercase leading-tight tracking-tighter text-[var(--foreground)]">
              {project.project}
            </h3>
            <p className="text-[10px] md:text-[10px] font-bold uppercase tracking-[0.2em] text-primary/80 mt-1">
              {project.descriptionTitle}
            </p>
          </div>
          <p className="text-sm leading-relaxed text-[var(--muted)] line-clamp-2 md:line-clamp-3 mb-6">
            {project.description}
          </p>
        ...
        {/* Image Side */}
        <div className="relative w-full sm:w-2/5 overflow-hidden p-6 md:p-8 flex items-center justify-center">
          <motion.div 
            className="relative h-full w-full transition-transform duration-500 group-hover:scale-110"
          >
            {/* Mobile: use aspect to ensure visibility in column */}
